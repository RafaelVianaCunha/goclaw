...
func (p *OpenAIProvider) buildRequestBody(model string, req ChatRequest, stream bool) map[string]any {
	...
	if v, ok := req.Options[OptTemperature]; ok {
		...
		// gpt-5.x flagship models (gpt-5.5, gpt-5.4, gpt-5.1) reject any temperature != 1;
		// only the default (1) is accepted, so skip sending it and let the API use its default.
		skipTemp := strings.HasPrefix(capabilityModel, "gpt-5.") || strings.HasPrefix(capabilityModel, "gpt-5-mini") || strings.HasPrefix(capabilityModel, "gpt-5-nano") || strings.HasPrefix(capabilityModel, "o1") || strings.HasPrefix(capabilityModel, "o3") || strings.HasPrefix(capabilityModel, "o4")
		if !skipTemp {
			body["temperature"] = v
		}
	}
